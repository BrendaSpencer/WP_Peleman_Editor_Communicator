...
class Editor_Meta_Variable_Input { 
	...
	woocommerce_wp_text_input(array(
        ...
        woocommerce_wp_checkbox(array(
            ...
            'wrapper_class'     => 'form-row form-row-first wsppe-form-row-padding-5',
            'custom_attributes' => array('foldout' => 'upload_' . $loop),
        ));
	...
	?>
  	...
  	<?php
            ...
            $Base_Meta->set_colorCode((string)(isset($post[Editor_Custom_Meta::PIE_COLOR_CODE_KEY][$loop]) ? $post[Editor_Custom_Meta::PIE_COLOR_CODE_KEY][$loop] : ''));
            $Base_Meta->set_backgroundId((string)(isset($post[Editor_Custom_Meta::PIE_BACKGROUND_ID_KEY][$loop]) ? $post[Editor_Custom_Meta::PIE_BACKGROUND_ID_KEY][$loop] : ''));

            $Base_Meta->set_price_per_extra_page((float)(isset($post[Editor_Custom_Meta::PRICE_PER_EXTRA_PAGE_KEY][$loop]) ? $post[Editor_Custom_Meta::PRICE_PER_EXTRA_PAGE_KEY][$loop] : 0.0));
			$Base_Meta->set_basePrice((float)(isset($post[Editor_Custom_Meta::BASE_PRICE_KEY][$loop]) ? $post[Editor_Custom_Meta::BASE_PRICE_KEY][$loop] : 1));
    	    $Base_Meta->set_default_page_amount((int)(isset($post[Editor_Custom_Meta::PAGE_AMOUNT_KEY][$loop]) ? $post[Editor_Custom_Meta::PAGE_AMOUNT_KEY][$loop] : 1));
    	    $Base_Meta->set_numPages((int)(isset($post[Editor_Custom_Meta::NUM_PAGES_KEY][$loop]) ? $post[Editor_Custom_Meta::NUM_PAGES_KEY][$loop] : 1));
            
            $Base_Meta->set_usesImageUpload((bool)(isset($post[Editor_Custom_Meta::USE_IMAGE_UPLOAD_KEY][$loop]) ? $post[Editor_Custom_Meta::USE_IMAGE_UPLOAD_KEY][$loop] : false));
            $Base_Meta->set_minImages((int)(isset($post[Editor_Custom_Meta::AUTOFILL_KEY][$loop]) ? $post[Editor_Custom_Meta::AUTOFILL_KEY][$loop] : 1));
            $Base_Meta->set_maxImages((int)(isset($post[Editor_Custom_Meta::MAX_IMAGES_KEY][$loop]) ? $post[Editor_Custom_Meta::MAX_IMAGES_KEY][$loop]: 1));
            $Base_Meta->set_autofill((bool)(isset( $post[Editor_Custom_Meta::MIN_IMAGES_KEY][$loop]) ? $post[Editor_Custom_Meta::MIN_IMAGES_KEY][$loop] : false));
  
            $Base_Meta->set_useProjectReference((bool)(isset($post[Editor_Custom_Meta::USE_PROJECT_REFERENCE_KEY][$loop]) ? $post[Editor_Custom_Meta::USE_PROJECT_REFERENCE_KEY][$loop] : false));
            $Base_Meta->set_overrideThumb((bool)(isset($post[Editor_Custom_Meta::OVERRIDE_CART_THUMB_KEY][$loop]) ? $post[Editor_Custom_Meta::OVERRIDE_CART_THUMB_KEY][$loop] :false));
            $Base_Meta->set_peleman_personalisation((string)(isset($post[Editor_Custom_Meta::PELEMAN_PERSONALISATION_KEY][$loop]) ? $post[Editor_Custom_Meta::PELEMAN_PERSONALISATION_KEY][$loop] : ''));
            $Base_Meta->set_editor_instructions((array)(isset($post[Editor_Custom_Meta::EDITOR_INSTRUCTIONS_KEY][$loop]) ? $post[Editor_Custom_Meta::EDITOR_INSTRUCTIONS_KEY][$loop] : []));
